...
async def write_weights(dut, weights):
    ...
    weight_width = dut.WEIGHT_EWIDTH.value + dut.WEIGHT_SIGWIDTH.value + 1
    ...
    for addr in range(dut.number_of_rows_per_frame.value):
        ...
        for i in range(dut.K.value):
            for j in range(dut.number_of_columns_per_frame.value):
                ...
                write_data += f'{weights[i][dut.number_of_columns_per_frame.value * addr + (dut.number_of_columns_per_frame.value - j - 1)].literal():0{weight_width}b}'
...
async def validate_multiplies(dut, row, weights):
    ...
    prod_width = dut.PROD_EWIDTH.value + dut.PROD_SIGWIDTH.value + 1
    ...
    for i in range(dut.K.value):
        ...
        for j in range(dut.number_of_columns_per_frame.value):
            ...
            expected_binstr = f'{expected.literal():0{prod_width}b}'
            expected = expected.as_float()
            assert expected == result, "{}, {}: {} * {}; result={} ({}) expected={} ({})".format(i, j, int(row[j]), weights[i][j].as_float(), result, result_binstr, expected, expected_binstr)
...
async def validate_tree(dut):
    ...
    prod_width = dut.PROD_EWIDTH.value + dut.PROD_SIGWIDTH.value + 1
    res_width = dut.EWIDTH.value + dut.SIGWIDTH.value + 1
    ...
    for i in range(dut.K.value):
        ...
        expected_binstr = f'{expected.literal():0{res_width}b}'
        expected = expected.as_float()
...
def calc_matvec(dut, frame, weights):
    ...
    for i in range(dut.K.value):
        ...
        for j in range(dut.number_of_rows_per_frame.value):
            ...
            while max > 1:
                temp = AFloatArray(np.zeros(dut.number_of_columns_per_frame.value)).as_config(config=prod_conf)
                for k in range(0, int(max / 2)):
                    temp[k] = mult_intermediates[i][j * dut.number_of_columns_per_frame.value + 2 * k].add(mult_intermediates[i][j * dut.number_of_columns_per_frame.value + 2 * k + 1], result_config=prod_conf)
...
def validate_output(dut, expected_vals):
    ...
    for i in range(dut.K.value):
        ...
        expected_binstr = f'{expected.literal():0{res_width}b}'
        expected = expected.as_float()
        assert expected == result, "{}: result={} ({}) expected={} ({})".format(i, result, result_binstr, expected, expected_binstr)
...
#     float_max_val = AFloat.from_literal(int('0' + '1' * (ebits + mbits), 2)).as_float()
#
...
#         result = AFloat.from_literal(int(dut.sum.value.binstr, 2)).as_float()
#
#         X_val = AFloat.from_literal(int(X_binstr, 2))
#         Y_val = AFloat.from_literal(int(Y_binstr, 2))
...
#                 f'Expected {expected.as_float()} ({format_binstr(expected_binstr)}), got {result} ({format_binstr(dut.sum.value.binstr)}) for {AFloat.from_literal(int(X_binstr, 2), prod_conf).as_float()} + {AFloat.from_literal(int(Y_binstr, 2), prod_conf).as_float()} ({format_binstr(X_binstr)}, {format_binstr(Y_binstr)})'
#
#
...
#     assert bitstr == conv, 'For "{}", got "{}" but expected "{}"'.format(val, conv, bitstr)
#     conv = AFloat.from_literal(int(bitstr, 2)).as_float()
#     assert val == conv, 'For "{}", got "{}" but expected "{}"'.format(bitstr, conv, val)
